...
namespace Barracuda.OpenApi.Services
{
    public class OpenApiReader : IOpenApiReader
    {
        ...
        public string Read(Assembly assembly, string controller, string server, string authUrl, string tokenUrl)
        {
            ...
            object[] attributes = methods[0].GetCustomAttributes(typeof(BOAVersion), false);

            if (attributes.Length > 0)
            {
                BOAVersion attribute = attributes[0] as BOAVersion;
            ...
            attributes = methods[0].GetCustomAttributes(typeof(BOAInfo), false);

            if (attributes.Length > 0)
            {
                BOAInfo attribute = attributes[0] as BOAInfo;
        ...
        void GetPaths(MethodInfo[] methods, IDictionary<string, object> paths, IDictionary<string, object> components)
        {
            ...
            {
                object[] attributes = method.GetCustomAttributes(typeof(BOA), false);

                if (attributes.Length > 0)
                {
                    attributes = method.GetCustomAttributes(typeof(BOAProducesResponse), false);

                    if (attributes.Length > 0)
                    {
                        BOAProducesResponse attribute = attributes[0] as BOAProducesResponse;
        ...
        {
            object[] attributes = method.GetCustomAttributes(typeof(BOARoute), false);

            if (attributes.Length > 0)
            {
                BOARoute attribute = attributes[0] as BOARoute;
        ...
        {
            object[] attributes = method.GetCustomAttributes(typeof(BOAHttpGet), false);
            ...
            {
                attributes = method.GetCustomAttributes(typeof(BOAHttpPost), false);
        ...
        {
            object[] attributes = method.GetCustomAttributes(typeof(BOASummary), false);

            if (attributes.Length > 0)
            {
                BOASummary attribute = attributes[0] as BOASummary;
        ...
        {
            object[] attributes = method.GetCustomAttributes(typeof(BOAFunctionName), false);

            if (attributes.Length > 0)
            {
                BOAFunctionName attribute = attributes[0] as BOAFunctionName;
        ...
        void GetParameters(MethodInfo method, IDictionary<string, Object> http)
        {
            ...
            object[] attributes = method.GetCustomAttributes(typeof(BOAParameter), false);

            foreach (var item in attributes)
            {
                BOAParameter attribute = item as BOAParameter;
        ...
        {
            object[] attributes = method.GetCustomAttributes(typeof(BOARequestBody), false);

            if (attributes.Length > 0)
            {
                BOARequestBody attribute = attributes[0] as BOARequestBody;
        ...
        void GetSecuritySchemas(IDictionary<string, Object> securitySchemes, string authUrl, string tokenUrl)
        {
            ...
            implicitSecurity.Add("scopes", new { 
                profile = "profile"
            });
